    ...
    while send_queues[chat_id]:
        ...
        try:
            ...
            keyboard = [
                ...
                [InlineKeyboardButton(get_text("repost_quiz", langを受けた

System: **الرد التلقائي مع الإصلاحات:**
...
PATTERNS = [
    re.compile(
        r"Q[.:)]?\s*(?P<q>.+?)\s*(?P<opts>(?:[A-J1-9][).:]\s*.+?(?:\n|$)){2,10})"
        r"(?:Answer|Ans|Correct Answer)[:：]?\s*(?P<ans>[新聞
    ...
    if args and args[0].startswith("quiz_"):
        ...
        if row:
            ...
            if chat_type == ChatType.CHANNEL:
                # إذا كان الرابط مستخدمًا في قناة
                send_queues[cid].append((q, opts, idx))
                asyncio.create_task(process_queue(cid, context, user_id=update.effective_user.id, is_private=False, quiz_id=quiz_id))
            else:
                # إذا كان في محادثة خاصة أو مجموعة، اطلب اختيار قناة
