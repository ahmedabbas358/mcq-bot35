...
SEND_INTERVAL = float(os.getenv("SEND_INTERVAL", "0.75"))
MAX_CONCURRENT_SEND = int(os.getenv("MAX_CONCURRENT_SEND", "5"))
...
semaphores: Dict[Target, asyncio.Semaphore] = defaultdict(lambda: asyncio.Semaphore(MAX_CONCURRENT_SEND))
sender_tasks: Dict[Target, asyncio.Task] = {}
...
async def get_user_settings(user_id: int) -> UserSettings:
    ...
    if row is None:
        ...
        await conn.execute(
            ...
            "user_id, default_target, default_target_title, delete_source, ai_enabled, ai_model, ai_count, share_mode, show_explanation, confirmation_message"
            ") VALUES (?, ?, ?, ?, 1, ?, ?, 'both', 1, ?)",
            (
                user_id,
...
async def generate_quizzes_with_ai(mode: str, payload: str, lang: str, count: int, model: str) -> List[Tuple[str, List[str], int, str]]:
...
def ensure_sender(target: Target, context: ContextTypes.DEFAULT_TYPE) -> None:
    if target not in sender_tasks or sender_tasks[target].done():
        sender_tasks[target] = context.application.create_task(_sender(target, context))


async def _sender(target: Target, context: ContextTypes.DEFAULT_TYPE) -> None:
    logger.info("Sender task started for target %s", target)
    try:
        while True:
                ...
                async with semaphores[target]:
                    try:
                        ...
                        owner_settings = await get_user_settings(item.owner_user_id) if item.owner_user_id else UserSettings(
                            None, "", DEFAULT_DELETE_SOURCE, True, OPENAI_MODEL, AI_DEFAULT_COUNT, "both", True, QUIZ_CONFIRMATION_MESSAGE
                        )
                        ...
                        if owner_settings.confirmation_message:
        ...
        logger.info("Sender task cancelled for %s", target)
        raise
...
async def enqueue_mcq(
    ...
    *,
    ...
    lang = infer_lang(getattr(message.from_user, "language_code", None), extract_message_text(message))
    try:
        settings = await get_user_settings(owner_user_id) if owner_user_id else UserSettings(
            None, "", DEFAULT_DELETE_SOURCE, True, OPENAI_MODEL, AI_DEFAULT_COUNT, "both", True, QUIZ_CONFIRMATION_MESSAGE
        )
    except Exception:
        settings = UserSettings(None, "", DEFAULT_DELETE_SOURCE, True, OPENAI_MODEL, AI_DEFAULT_COUNT, "both", True, QUIZ_CONFIRMATION_MESSAGE)

    target = explicit_target or settings.default_target or message.chat.id
    raw_text = text_override if text_override is not None else extract_message_text(message)
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not message:
        return
    lang = infer_lang(getattr(update.effective_user, "language_code", None), extract_message_text(message))
...
async def help_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not message or not update.effective_user:
        return
    lang = infer_lang(update.effective_user.language_code, extract_message_text(message))
...
async def stats_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.effective_message or not update.effective_user:
        return
    lang = infer_lang(update.effective_user.language_code, extract_message_text(update.effective_message))
...
async def setchannel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not message or not user or not chat:
        return
    lang = infer_lang(user.language_code, extract_message_text(message))
...
async def health_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    active_targets = len(
        ...
        | {target for target, task in sender_tasks.items() if not task.done()}
    )
    text = get_text(
        "health",
        lang,
...
async def run_ai_flow(
    *,
    ...
    try:
        quizzes = await generate_quizzes_with_ai(mode, clean_payload, lang, count, settings.ai_model)
        target = explicit_target or settings.default_target or message.chat.id
        queued = await enqueue_quiz_items(
            target=target,
            quizzes=quizzes,
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    data = query.data or ""
    user = update.effective_user
    lang = infer_lang(getattr(user, "language_code", None), "")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    lang = infer_lang(getattr(user, "language_code", None), raw_text)
...
async def on_shutdown(app) -> None:
    ...
    for task in list(sender_tasks.values()):
        task.cancel()
    if sender_tasks:
